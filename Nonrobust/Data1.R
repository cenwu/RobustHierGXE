Data1 <- function(n,s)
{
    ...
    # simulate the environment factors 
         for (i in 1: q)
            {
                   for (j in 1: q)
                    {
                       sige[i,j] = 0.5^abs(i-j)
                    }           
         ...
         for (i in 1: s)
            {
                   for (j in 1: s)
                    {
                       sig[i,j] = 0.5^abs(i-j)                                         # AR
                      # sig[i,j] = 1*(i==j) + 0.5*(abs(i-j)==1) + 0.33*(abs(i-j)==2)   # Banded
                    }           
...
####################################################################################
     ...
     for ( i in 2:n )
      ...
      xx = cbind(ez,xx)
     dat = list(y= Y, x= xz, e= ez, xx=xx, rate=1-sum(status)/n, status=status)
     return(dat)    
