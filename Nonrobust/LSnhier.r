...
LSnhier <- function(y,x,e,y.test,x.test,e.test,tol,eps,s,xx,xx.test)  
{     

      
           lambda1 =    seq(160,600, by = 50)/(1*10^15) ; nlambda1 = length(lambda1);
           ...
           beta0 <- as.matrix(as.vector(predict(lasso.fit1, s=alpha <- lasso.cv$lambda.min, type="coefficients"))[-1])    
             ...
             {       
              ...
              for (j in 1:nlambda2)
                {      
                   lam1 = lambda1[i]    
                   lam2 = lam1   
                   if (i==1) {  beta = beta0 }  # use warm start 
                   beta = beta0                 
                   res = res0                  
                   diff.out.old = 2 
                   diff.in = diff.out = 1 
                   step.in = step.out = 0
                   while(diff.out>=10^{-4} & step.out<=20 ) 
                      {
                            beta.out = beta
                                 for(m in 1:s) 
                                     {
